...
Text::Text(GLuint width, GLuint height, const GLchar* vPath, const GLchar* fPath) : shader(vPath, fPath){
    // Set OpenGL options
...
void Text::GenerateText(const GLchar *path) {
    FT_Library ft;
...
void Text::RenderText(std::string text, GLfloat xPos, GLfloat yPos, GLfloat scale, glm::vec3 color) {
    ...
    for (c = text.begin(); c != text.end(); c++)
    {
        ...
        GLfloat xpos = xPos + ch.Bearing.x * scale;
        GLfloat ypos = yPos - (ch.Size.y - ch.Bearing.y) * scale;
